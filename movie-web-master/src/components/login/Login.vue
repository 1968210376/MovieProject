<template>
  <div class="login_container">
    <div>
      <h3 class="title">video-web-master 视频管理系统</h3>
    </div>

    <div class="login_box">
      <h3 style="text-align:center;color: seashell;">登录</h3>
      <!-- 登陆表单区域 -->
  ...
  methods: {
    ...
    login () {
      this.$refs.loginFormRef.validate(async valid => {
        if (!valid) return
        const { data: res } = await this.$axios.post('api/logins', this.$qs.stringify(this.loginForm))
