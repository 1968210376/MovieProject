    ...
    <el-container class="home-container">
      <el-header>
        <el-menu class="menu"
          ...
          <!-- 一级菜单 无子菜单 -->
          <el-menu-item :index="item.t_enname"
                        v-for="item in noChildren"
                        @click="saveNavState(item.t_enname),saveNavId(item.t_id),saveNavName(item.t_name)"
                        :key="item.t_id">
            <!-- 一级菜单的模板 -->
            <template slot="title">
              <!-- 文本 -->
          ...
          <!-- 一级菜单 有子菜单 -->
          <el-submenu :index="item.t_enname"
                      v-for="item in hasChildren"
                      @click="saveNavState(item.t_enname)"
                      :key="item.t_id">
            <!-- 一级菜单的模板 -->
            <template slot="title">
              <!-- 文本 -->
            ...
            <!-- 二级菜单 -->
            <el-menu-item :index="subItem.t_enname + ''"
                          v-for="subItem in item.children"
                          :key="subItem.t_id + ''"
                          @click="saveNavState(subItem.t_enname),saveNavId(subItem.t_id),saveNavName(subItem.t_name)">
              <!-- 二级菜单的模板 -->
              <template slot="title">
                <!-- 文本 -->
                <span>{{(subItem.t_name)}}</span>
              </template>
            </el-menu-item>
          </el-submenu>
          ...
          <el-popover placement="bottom"
            ...
            <el-button @click="drawer = true"
                       type='primary'>用户信息</el-button>
            <el-button @click="logout">退出</el-button>
            <el-button style="position:absolute;right:0px;top:-7px;"
                       slot="reference">
              <el-avatar src="https://cube.elemecdn.com/0/88/03b0d39583f48206768a7534e55bcpng.png">
              </el-avatar>
            </el-button>
          </el-popover>
          <el-drawer title="用户信息管理"
                     :visible.sync="drawer"
                     :direction="direction"
      ...
      <el-footer>
        <div class="banquan">
          <p> Design By <span class="ban">Dang Zhiguo</span>
            <!-- <span>友情链接：</span> <a @click="linkDownload('http://www.jiugev.xyz/h5')">www.jiugev.xyz/h5</a> || -->
            <span> 备案号：</span><a href="https://beian.miit.gov.cn/"
               target="_blank">豫ICP备2022007638</a>
          </p>
          <p>本站不提供任何视听上传储存服务，所有内容均来至网络自动采集，只提供web页面浏览服务，并不提供影片资源存储，也不参与任何视频录制、上传.</p>

          <p> 若本站收录的节目无意侵犯了贵司权利，请给我们来信，我们会及时处理和删除，谢谢！Copyright © 2022 movie-web</p>
        </div>
